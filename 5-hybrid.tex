...
%\caption{{\bf Word-based NMT} -- the backbone of my hybrid architecture. Boxes
...
%that I will expand next to accomplish the full architecture as 
illustrated in Figure~\ref{f:hybrid}.
%and are highlighted by the two boxes in 
%Figure~\ref{f:word}.
%Note that I leave \unk{} on the target input which helps simplify the training
...
worlds. 
%I illustrate that unified view in Figure~\ref{f:vocabulary}.


\subsection{Source Character-based Representation}
\label{subsec:src}
%I now describe box (a) in Figure~\ref{f:word}. 
...
of source words. 
%In my running example, % for the word \word{cute}, 
...
later that I can train my deep character-level LSTM %in the experiment section 
...
%for rare words. Their work, however, is different from mine in that the representations
...
%approaches zero, I obtain character-based NMT; whereas, large vocabulary
...
\label{subsec:tgt}
%Next, I describe box (b) in Figure~\ref{f:word}.
...
target vocabulary. 
%In my running example, the predicted characters are .
...
%\caption{{\bf Target generation for rare words} -- the backbone of my hybrid architecture. Boxes
%mark the character-level components for: (a) computing source word
%representations and (b) generating target words.}
%%to take advantages of the
%%original forms of the rare words.}
...
%and I can split these computations into
...
word-level NMT. Such translations contains \unk{} tokens, so I utilize my
character-level decoder with beam search to generate actual words for these \unk{}.
%I aggregate the word- and charater-level
...
When the vocabulary size is large, i.e., when my hybrid NMT system is closer to a
...
that my hybrid NMT may have ignored valuable information about the
...
The number of frequent words I sample per 
mini-batch is counted by {\it type} on the source side  and by {\it
token} on the target side. This is in accordance with how I sample rare words
...
%Unless otherwise stated, the above settings are used for most of my systems. 
...
%character-based NMT, I use a vocabulary of 200 characters per language. 
I
...
%I present results for some of my models and 
...
%Lastly, % for my {\it hybrid} models, 
%results in Table~\ref{t:encs} justify why my proposed {\it hybrid} 
...
%models, my word-character approach is preferred.
...
%This proves that my hybrid model has successfully replaced the
...
%\footnote{Note that, my other models
%combining both the separate-path approach and the 2-layer character-level
%components have not converged yet. I expect to obtain even better results in
%the next version of my paper.}
...
%vocabulary size to see at which point I stop benefiting from the hybrid
